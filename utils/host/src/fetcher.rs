...
use alloy_consensus::{BlockHeader, Header, Transaction, TxEnvelope};
use alloy_eips::{BlockId, BlockNumberOrTag};
use alloy_primitives::{keccak256, Address, Bytes, FixedBytes, B256, U256, U64};
...
use kona_proof::sync::compute_origin;
use kona_protocol::{L2BlockInfo, Predeploys};
...
impl OPSuccinctDataFetcher {
    ...
    pub async fn get_batcher_sender_info_at(&self, block_id: BlockId) -> Result<(Address, u64)> {
        let Some(rollup_config) = self.rollup_config.as_ref() else {
            return Err(anyhow::anyhow!("Rollup config not loaded."));
        };

        let batcher_sender_address = self
            .l1_provider
            .get_storage_at(
        ...
        let tx_count = self
            .l1_provider
            .get_transaction_count(batcher_sender_address)
            .block_id(block_id)
            .await?;

        Ok((batcher_sender_address, tx_count))
    }
    ...
    pub async fn get_l1_traversal_stats(
        &self,
        l2_start_block: u64,
        l1_head_hash: B256,
        ...
        let Some(mut system_config) = rollup_config.genesis.system_config else {
            return Err(anyhow::anyhow!("System config not loaded."));
        };
        ...
        let l2_claim_block_info = L2BlockInfo::from_block_and_genesis(
            &start_block.into_consensus(),
            &rollup_config.genesis,
        )?;

        let l1_origin = compute_origin(
            rollup_config,
            l2_claim_block_info.l1_origin.number,
            l2_claim_block_info,
        );

        let l1_head = self
            .l1_provider
            .get_block(BlockId::hash(l1_head_hash))
            .await?
            .ok_or_else(|| anyhow::anyhow!("L1 head not found with hash {}", l1_head_hash))?;

        let (mut tx_count, mut da_tx_count) = (0, 0);
        for i in l1_origin..=l1_head.header.number {
            let block = self
                .l1_provider
                .get_block(BlockId::number(i))
                .full()
                .await?
                .ok_or_else(|| anyhow::anyhow!("Couldn't find block number: #{i}"))?;

            let receipts = self
                .l1_provider
                .get_block_receipts(BlockId::number(i))
                .await?
                .ok_or_else(|| anyhow::anyhow!("Receipts not found for block #{}", i))?;
            ...
            system_config.update_with_receipts(
                receipts
                    ...
                    .map(|r| {
                        r.inner
                            .map_logs(alloy_primitives::Log::from)
                            .as_receipt()
                            .cloned()
                            .unwrap()
                    })
                    ...
                    .as_slice(),
                rollup_config.l1_system_config_address,
                rollup_config.is_ecotone_active(block.header.timestamp),
            )?;
            ...
            da_tx_count += block
                ...
                .filter(|tx| {
                    ...
                    ) && tx.to() == Some(rollup_config.batch_inbox_address)
                        && tx.inner.signer() == system_config.batcher_address
                })
                .count()
        }

        Ok((
            l1_head.header.number,
            l1_origin,
            tx_count as u64,
            da_tx_count as u64,
        ))
    ...
    pub async fn get_host_args(
        ...
        let l2_agreed_withdrawal_root: FixedBytes<32> = if self
            .rollup_config
            .as_ref()
            .unwrap()
            .is_tenrec_active(l2_output_block.header.timestamp)
        {
            l2_provider
                .get_storage_at(
        ...
        let l2_output_encoded = L2Output {
            ...
            l2_storage_hash: l2_agreed_withdrawal_root,
            l2_claim_hash: agreed_l2_head_hash.0.into(),
        };
        let agreed_l2_output_root = keccak256(l2_output_encoded.abi_encode());

        // We check that the recomputed preroot matches the one op-node has
        let pre_root = self
            .get_l2_output_at_block(l2_start_block)
            .await?
            .output_root;
        assert_eq!(pre_root, agreed_l2_output_root);
        ...
        let l2_claim_withdrawal_root: FixedBytes<32> = if self
            .rollup_config
            .as_ref()
            .unwrap()
            .is_tenrec_active(l2_claim_block.header.timestamp)
        {
            l2_provider
                .get_storage_at(
                    Predeploys::L2_TO_L1_MESSAGE_PASSER,
        ...
        let l2_claim_encoded = L2Output {
            ...
            l2_storage_hash: l2_claim_withdrawal_root,
            l2_claim_hash: l2_claim_hash.0.into(),
        };
        let claimed_l2_output_root = keccak256(l2_claim_encoded.abi_encode());
        // We check that the recomputed postroot matches the one op-node has
        let post_root = self.get_l2_output_at_block(l2_end_block).await?.output_root;
        assert_eq!(post_root, claimed_l2_output_root);

        let (start_l1_head_hash, _) = self.get_l1_head(l2_start_block, false).await?;
        let (end_l1_head_hash, _) = self.get_l1_head(l2_end_block, false).await?;

        let (agreed_sender_address, agreed_nonce) = self
            .get_batcher_sender_info_at(BlockId::hash(start_l1_head_hash))
            .await?;

        let (claimed_sender_address, claimed_nonce) = self
            .get_batcher_sender_info_at(BlockId::hash(end_l1_head_hash))
            .await?;
