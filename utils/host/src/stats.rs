...
pub struct ExecutionParams {
    ...
    pub l1_head: u64,
    pub l1_origin: u64,
    pub l1_txs_origin_to_head: u64,
    pub l1_da_txs_origin_to_head: u64,
    pub batch_start: u64,
    pub batch_end: u64,
...
pub struct ExecutionStats {
    ...
    pub transaction_verification_count: u64,
    pub receipt_verification_count: u64,
    pub block_info_count: u64,
...
impl fmt::Display for ExecutionStats {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write_stat(f, "Transaction Cycles", self.transaction_verification_count)?;
        write_stat(f, "Block Info Cycles", self.block_info_count)?;
        write_stat(f, "Receipt Cycles", self.receipt_verification_count)?;
        write_stat(
            f,
...
impl ExecutionStats {
    ...
    pub fn new(
        l1_origin: u64,
        l1_head: u64,
        l1_txs_origin_to_head: u64,
        l1_da_txs_origin_to_head: u64,
        ...
        Self {
            ...
            transaction_verification_count: get_cycles("transactions"),
            receipt_verification_count: get_cycles("receipt"),
            block_info_count: get_cycles("block-info"),
    ...
    pub fn new_raw(
        ...
        let nb_blocks = execution_params.batch_end - execution_params.batch_start;

        let (cycles_per_transaction, gas_used_per_transaction) = if execution_params.is_agg {
            (0, 0)
        } else {
            let cycles_per_transaction =
                report.total_instruction_count() / execution_params.nb_transactions;
            let gas_used_per_transaction =
                execution_params.eth_gas_used / execution_params.nb_transactions;
            (cycles_per_transaction, gas_used_per_transaction)
        };

        Self {
            l1_origin: execution_params.l1_origin,
            l1_head: execution_params.l1_head,
            l1_da_txs_origin_to_head: execution_params.l1_da_txs_origin_to_head,
            l1_txs_origin_to_head: execution_params.l1_txs_origin_to_head,
            batch_start: execution_params.batch_start,
            batch_end: execution_params.batch_end,
            total_instruction_count: report.total_instruction_count(),
            total_sp1_gas: report.gas.unwrap_or(0),
            block_execution_instruction_count: get_cycles("block-execution"),
            oracle_verify_instruction_count: get_cycles("oracle-verify"),
            ...
            ec_recover_cycles: get_cycles("precompile-ec-recover"),
            p256_verify_cycles: get_cycles("precompile-p256-verify"),
            nb_transactions: execution_params.nb_transactions,
            eth_gas_used: execution_params.eth_gas_used,
            l1_fees: execution_params.l1_fees,
            total_tx_fees: execution_params.total_tx_fees,
            nb_blocks,
            cycles_per_block: report.total_instruction_count() / nb_blocks,
            cycles_per_transaction,
            transactions_per_block: execution_params.nb_transactions / nb_blocks,
            gas_used_per_block: execution_params.eth_gas_used / nb_blocks,
            gas_used_per_transaction,
            witness_generation_time_sec: execution_params.witness_generation_time_sec,
            total_execution_time_sec: execution_params.total_execution_time_sec,
            high_memory_threshold,
        }
    }

    pub fn use_high_memory(&self) -> bool {
        if let Some(high_memory_threshold) = self.high_memory_threshold {
            self.total_instruction_count > high_memory_threshold
        } else {
            false
        }
    }
