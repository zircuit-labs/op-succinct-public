use std::{os::unix::fs::PermissionsExt as _, str::FromStr};
...
impl KeystoreS3Config {
    fn from_env() -> anyhow::Result<Self> {
        Ok(Self {
            bucket: std::env::var("KEYSTORE_S3_BUCKET").context("missing KEYSTORE_S3_BUCKET")?,
            region: std::env::var("KEYSTORE_S3_REGION").context("missing KEYSTORE_S3_REGION")?,
            endpoint: std::env::var("KEYSTORE_S3_ENDPOINT")
                .context("missing KEYSTORE_S3_ENDPOINT")?,
            force_path_style: std::env::var("KEYSTORE_S3_FORCE_PATH_STYLE")
                .unwrap_or("false".to_owned())
                .parse()
                .context("KEYSTORE_S3_FORCE_PATH_STYLE must be bool")?,
            file: std::env::var("KEYSTORE_S3_FILE").context("missing KEYSTORE_S3_FILE")?,
            passphrase: std::env::var("KEYSTORE_S3_PASSPHRASE")
                .context("missing KEYSTORE_S3_PASSPHRASE")?,
        })
    }
}
...
impl Signer {
    ...
    pub async fn from_env() -> Result<Self> {
            ...
            (std::env::var("SIGNER_URL"), std::env::var("SIGNER_ADDRESS"))
        {
            ...
            tracing::info!("Neither (SIGNER_URL and SIGNER_ADDRESS) nor PRIVATE_KEY are set in environment, assuming S3 keystore");
            let keystore_cfg = KeystoreS3Config::from_env()?;
            let tmp_keystore = tempfile::Builder::new()
                .permissions(std::fs::Permissions::from_mode(OWNER_RW))
                .tempfile()?;

            let config = aws_config::defaults(BehaviorVersion::v2025_01_17())
                .endpoint_url(keystore_cfg.endpoint)
                .region(Region::new(keystore_cfg.region));

            let force_path_style = keystore_cfg.force_path_style;
            ...
            let response = client
                .get_object()
                .bucket(&keystore_cfg.bucket)
                .key(keystore_cfg.file)
                .send()
                .await
                .context("Failed to download keystore from S3")?;

            let data = response
                .body
                .collect()
                .await
                .context("Failed to read keystore data from S3 response")?;
            let path = tmp_keystore.path();

            tokio::fs::write(&path, data.into_bytes().to_vec())
                .await
                .context("Failed to write keystore to temporary file")?;

            let private_key = PrivateKeySigner::decrypt_keystore(path, &keystore_cfg.passphrase)
                .context("Failed to decrypt keystore with provided passphrase")?;
            Ok(Signer::LocalSigner(private_key))
        }
    }
...
mod tests {
    ...
    async fn test_send_transaction_request() {
        let proposer_signer = Signer::Web3Signer(
            ...
            "0x9b3F173823E944d183D532ed236Ee3B83Ef15E1d"
                .parse()
                .unwrap(),
        );
