...
// ABI encoding of AggregationOutputs is 8 * 32 bytes.
...
sol! {
    ...
    struct BootInfoStruct {
        bytes32 l1Head;
        uint64 preNonce;
        address preSenderAddress;
        uint64 postNonce;
        address postSenderAddress;
        bytes32 l2PreRoot;
        bytes32 l2PostRoot;
...
impl From<BootInfo> for BootInfoStruct {
    fn from(boot_info: BootInfo) -> Self {
        BootInfoStruct {
            ...
            preNonce: boot_info.agreed_nonce,
            preSenderAddress: boot_info.agreed_sender_address,
            postNonce: boot_info.claimed_nonce,
            postSenderAddress: boot_info.claimed_sender_address,
