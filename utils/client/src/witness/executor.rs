...
pub trait WitnessExecutor {
        ...
        P: Pipeline + SignalReceiver + Send + Sync + Debug,
    {
        ...
        let (final_address, final_nonce) = driver
            .pipeline
            .final_nonce()
            .ok_or_else(|| anyhow!("Failed to get final nonce"))?;

        if final_address != boot.claimed_sender_address {
            return Err(anyhow!(
                "Failed to validate L2 block #{number} with claimed sender address {claimed_sender_address}. Got {final_address} instead",
                number = safe_head.block_info.number,
                final_address = final_address,
                claimed_sender_address = boot.claimed_sender_address,
            ));
        }

        if final_nonce != boot.claimed_nonce {
            return Err(anyhow!(
                "Failed to validate L2 block #{number} with claimed post nonce {claimed_post_nonce}. Got {final_nonce} instead",
                number = safe_head.block_info.number,
                final_nonce = final_nonce,
                claimed_post_nonce = boot.claimed_nonce,
            ));
        }
