...
CREATE INDEX IF NOT EXISTS idx_requests_commitment_chain
ON requests (range_vkey_commitment, rollup_config_hash, l1_chain_id, l2_chain_id);

CREATE INDEX IF NOT EXISTS idx_requests_status_type
ON requests (status, req_type, start_block);

CREATE INDEX IF NOT EXISTS idx_requests_agg_commitment
ON requests (range_vkey_commitment, rollup_config_hash, aggregation_vkey_hash, l1_chain_id, l2_chain_id);

CREATE INDEX IF NOT EXISTS idx_requests_chain_locks
ON requests (l1_chain_id, l2_chain_id);
