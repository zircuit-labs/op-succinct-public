...
pub fn main() {
    ...
    agg_inputs.boot_infos.windows(2).for_each(|pair| {
        ...
        // The sender address of the previous boot info must be the sender address of the current
        // boot.
        assert_eq!(prev_boot_info.postSenderAddress, boot_info.preSenderAddress);

        // The claimed nonce of the previous boot info must be the pre nonce of the current
        // boot.
        assert_eq!(prev_boot_info.postNonce, boot_info.preNonce);
    });
    ...
    let final_boot_info = BootInfoStruct {
        ...
        preNonce: first_boot_info.preNonce,
        preSenderAddress: first_boot_info.preSenderAddress,
        postNonce: last_boot_info.postNonce,
        postSenderAddress: last_boot_info.postSenderAddress,
    ...
    let agg_outputs = AggregationOutputs {
        ...
        preNonce: final_boot_info.preNonce,
        preSenderAddress: final_boot_info.preSenderAddress,
        claimNonce: final_boot_info.postNonce,
        claimSenderAddress: final_boot_info.postSenderAddress,
        l2PreRoot: final_boot_info.l2PreRoot,
        claimRoot: final_boot_info.l2PostRoot,
        claimBlockNum: final_boot_info.l2BlockNumber,
        rollupConfigHash: final_boot_info.rollupConfigHash,
        rangeVkeyCommitment: multi_block_vkey_b256,
