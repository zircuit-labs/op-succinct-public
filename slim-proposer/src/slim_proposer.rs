use crate::nats::message_bus::{NatsBusConfig, NatsMessageBus};
use crate::nats::{MessageBus, ProposerMessage};
...
const NEXT_MSG_SLEEP_DURATION: Duration = Duration::from_millis(2000);
const NEXT_MESSAGE_TIMEOUT: Duration = Duration::from_millis(25_000);
...
impl SlimProposer {
    ...
    pub async fn run_loop(&mut self) -> Result<()> {
        loop {
            match timeout(NEXT_MESSAGE_TIMEOUT, self.nats.next_message()).await {
                Ok(Ok(Some(message))) => {
                    info!("Received message from NATS: #{}", message.id());
                    self.process_message(message).await;
                }
                Ok(Ok(None)) => {
                    tokio::time::sleep(NEXT_MSG_SLEEP_DURATION).await;
                }
                Ok(Err(err)) => {
                    error!("Error fetching next NATS message: {err:?}");
                    // the NATS client should try reconnecting on its own if we get disconnected
                    // if this fails after max attempts, we should bail and let the service be recreated
                    anyhow::bail!(err)
                }
                Err(_) => {
                    error!("Fetching next message from NATS timed out after {}s. Trying to force reconnecting.", NEXT_MESSAGE_TIMEOUT.as_secs());
                    self.nats.try_reconnecting().await?;
                }
            }
        }
    }
