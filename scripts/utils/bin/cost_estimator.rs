...
async fn execute_blocks_and_write_stats_csv<H: OPSuccinctHost>(
    ...
    let block_data = futures::stream::iter(ranges.clone())
        .zip(futures::stream::iter(host_args))
        .map(|(range, host_arg)| async {
            let block_data = data_fetcher
                .get_l2_block_data_range(range.start, range.end)
                .await
                .expect("Failed to fetch block data range.");
            let head_hash = host.get_l1_head_hash(host_arg).unwrap();
            let l1_stats = data_fetcher
                .get_l1_traversal_stats(range.start, head_hash)
                .await
                .unwrap();
            (range, block_data, l1_stats)
        })
        .buffered(15)
        .collect::<Vec<_>>()
        .await;
    ...
    let report_path = root_dir.join(format!(
        "execution-reports/{l2_chain_id}/{start}-{end}-report.csv"
    ));
    ...
    execution_inputs.par_iter().for_each(|(sp1_stdin, (range, block_data, l1_stats))| {
        let (l1_head, l1_origin, l1_txs_origin_to_head, l1_da_txs_origin_to_head) = *l1_stats;
        ...
        let execution_stats = ExecutionStats::new(l1_origin, l1_head, l1_txs_origin_to_head, l1_da_txs_origin_to_head, block_data, &report, 0, 0, None);

        // Save the Sp1Stdin for Sindri Proof Benchmarking:
        let input_path = root_dir.join(format!(
            "execution-reports/{}/{}-{}-stdin.bin",
            l2_chain_id, range.start, range.end
        ));
        fs::write(
            input_path,
...
fn aggregate_execution_stats(
    ...
    for stats in execution_stats {
        ...
        aggregate_stats.transaction_verification_count += stats.transaction_verification_count;
        aggregate_stats.receipt_verification_count += stats.receipt_verification_count;
        aggregate_stats.block_info_count += stats.block_info_count;
